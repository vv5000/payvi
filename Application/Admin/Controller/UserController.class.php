...
class UserController extends BaseController
{
    ...
    public function recharge()
    {
        ...
        $where   = array();
        $where['lx'] = array('in','3,4,13,23,33');   //手动增加减少  13 商户增加审核通过，23商户增加未审核,33审核不通过
        $orderid = I("get.orderid");

        if ($orderid) {
            $where['userid'] = array('eq', $orderid);
        }
        $this->assign('orderid', $orderid);

        $bank = I("request.bank", '');
        if ($bank && $bank=13) {
            $where['lx'] = array('in', '13,23');
        }elseif ($bank) {
        ...
        $todaysumMap['datetime'] = ['between', [$todayBegin, $todyEnd]];
        if ($orderid) {
            $todaysumMap['userid'] = array('eq', $orderid);
        }



        $todaysumMap['lx'] = ['in', '3,13'];
        $stat['today_sum_add'] = M('Moneychange')->field("sum(money) as money,sum(sxmoney) as sxmoney")->where($todaysumMap)->find();
        $todaysumMap['lx'] = ['in', '4'];
        $stat['today_sum_sub'] = M('Moneychange')->field("sum(money) as money,sum(sxmoney) as sxmoney")->where($todaysumMap)->find();
        //月
        $monthBegin = date('Y-m-0').' 00:00:00';
        $monthEnd = date('Y-m-31').' 23:59:59';
        $monthsumMap['datetime'] = ['between', [$monthBegin, $monthEnd]];
        if ($orderid) {
            $monthsumMap['userid'] = array('eq', $orderid);
        }
        $monthsumMap['lx'] = ['in', '3,13'];
        $stat['month_sum_add'] = M('Moneychange')->field("sum(money) as money,sum(sxmoney) as sxmoney")->where($monthsumMap)->find();
        $monthsumMap['lx'] = ['in', '4'];
        $stat['month_sum_sub'] = M('Moneychange')->field("sum(money) as money,sum(sxmoney) as sxmoney")->where($monthsumMap)->find();
        //All
        if ($orderid) {
...
//商户提交充值审核
    public function editRechargeStatus()
    {
        ...
        $info = M('Moneychange')->where(['id' => $id])->find();
        if (IS_POST) {
            ...
            $MoneychangeStatus  = $Moneychange->where(['id' => $id])->lock(true)->find();   //加锁
            if (empty($MoneychangeStatus)) {
                $this->ajaxReturn(['status' => 0, 'msg' => '充值申请不存在']);
            }
            //加锁用户信息
            $userinfo = M("Member")->where(["id" => $info['userid']])->lock(true)->find();
            if (empty($userinfo)) {
            ...
            if ($lx == 13) {
                $data["balance"] = array('exp', "balance+" . $info['money']);
                $gmoney          = $userinfo['balance'] + $info['money'];
                $arrayField  = array(
                    'ymoney'     => $userinfo['balance'],
                    "gmoney"     => $gmoney,
                    "datetime"   => date("Y-m-d H:i:s"),
                    "tongdao"    => '',
                    "lx"         => $lx, // 增减类型
                    "contentstr" => $info['contentstr'] . '【审核时间:' . date("Y-m-d H:i:s") . '】'.$memo,
                );

                $where['id'] = $info['userid'];
                $res1=M('Member')->where($where)->save($data);      //修改用户金额
                $res2=M('Moneychange')->where(['id' => $id])->save($arrayField);   //更新充值记录

                $parentid=$info['userid'];
                $this->deal_yongjin($info, $parentid, 3);
                ...
                $arrayField  = array(
                    "lx"         => $lx, // 增减类型
                    "contentstr" => $info['contentstr'] . '【驳回时间:' . date("Y-m-d H:i:s") . '】'.$memo,
                );
                $res2=M('Moneychange')->where(['id' => $id])->save($arrayField);
                if ($res2) {
                    M()->commit();
                    $this->ajaxReturn(['status' => 1, 'msg' => '操作成功']);
                } else {
     ...
     * @return bool
     * @author: 秋枫红叶
     * @date: 2020/7/13  18:39
     */
    public function deal_yongjin($info,$userid,$num)
    {
        if ($num <= 0) {
            return false;
        }
        //读取上级代理ID
        $where['id'] = $userid;
        $recharge_money=$info['money']+$info['sxmoney'];
        $member_info = M('Member')->field('parentid,recharge_rate')->where($where)->find();
        $parentid = $member_info['parentid'];
        $member_rate = $member_info['recharge_rate'];
        if ($parentid > 1) {   //上级ID大于1 执行此处代码
            $parent = M('Member')->where(['id' => intval($parentid)])->find();
            $parent_rate = $parent['recharge_rate'];
            $ratediff = bcsub($member_rate ,$parent_rate,4);   //费率差
            if ($ratediff >0) {   //费率差大于0
                $brokerage = bcmul($recharge_money , $ratediff,4);
                $rows = [
                ...
                $arrayField = array(
                    ...
                    "orderid"  => $info['transid'],
                    "tcuserid" =>$userid,
                    "contentstr" => '下游用户'.$info['userid'].'自助充值'.$recharge_money.'元，获得提成！',
                    "tcdengji" => 1,
                    "lx"       => 9,
                );
                moneychangeadd($arrayField); // 资金变动记录
            }else{
                return false;
            }
            $num = $num - 1;
            $this->deal_yongjin($info,$parentid,$num);
        }else{
            return false;
        }
    }
    ...
    public function incrMoney()
    {
        ...
        $s = session('tjtime');
        if($s && time() - $s <5 ){
            $this->ajaxReturn(['status' => 0, 'msg' => '请稍候提交']);
        }
    ...
    public function frozenHandle()
    {
        if (IS_POST) {
            ...
            $Model->startTrans();
            //更新资金
            $upRes = $Model->table('pays_member')->where(['id' => $blockData['userid']])->save($rows);
            //更新状态
            $uplog = $Model->table('pays_blockedlog')->where(array('id' => $blockData['id']))->save(array('status' => 1));
            //增加记录
            $data               = array();
            ...
            $change             = $Model->table('pays_moneychange')->add($data);
    ...
    public function unfreeze()
    {
        if (IS_POST) {
            ...
            $Model->startTrans();
            //更新资金
            $upRes = $Model->table('pays_member')->where(['id' => $blockData['user_id']])->save($rows);
            //更新状态
            $uplog = $Model->table('pays_auto_unfrozen_order')->where(array('id' => $blockData['id'], 'status' => 0))->save(array('status' => 1, 'real_unfreeze_time' => time()));
            //增加记录
            $data               = array();
    ...
    public function thawingFunds()
    {
        ...
        if ($data) {
            foreach ($data as $item) {
                ...
                //更新资金
                $upRes = $Model->table('pays_member')->where(['id' => $item['userid']])->save($rows);

                //更新状态
                $uplog = $Model->table('pays_blockedlog')->where(array('id' => $item['id']))->save(array('status' => 1));
