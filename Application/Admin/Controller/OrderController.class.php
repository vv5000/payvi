...
class OrderController extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($memberid) {
            ...
            $todaysumMap['pays_memberid'] =  $monthsumMap['pays_memberid'] = $nopaidsumMap['pays_memberid'] =  $monthNopaidsumMap['pays_memberid'] = array('eq', $memberid);
        ...
        if ($bank) {
            $where['O.pays_bankcode'] = array('eq', $bank);
        }
        ...
        // exit;
        if ($payOrderid) {
            $where['O.pays_orderid'] = array('eq', $payOrderid);
            $profitMap['transid'] = $payOrderid;
        }
        $this->assign('payOrderid',$payOrderid);
        $body = I("request.body", '', 'strip_tags');
        if ($body) {
            $where['O.pays_productname'] = array('eq', $body);
        }
        $this->assign('body', $body);
        $status = I("request.status");
        if ($status != "") {
            if ($status == '1or2') {
                $where['O.pays_status'] = array('between', array('1', '2'));
            } else {
                $where['O.pays_status'] = array('eq', $status);
            }
        }
        ...
        $sum['pays_poundage'] = $sum['pays_poundage'] - $sum['cost'] - $sum['memberprofit'];//原始
        foreach ($sum as $k => $v) {
            $sum[$k] += 0;
        ...
        if ($status == '1or2' || $status == 1 || $status == 2) {
            ...
            $todaysumMap['pays_successdate'] = ['between', [strtotime($todayBegin), strtotime($todyEnd)]];
            $todaysumMap['pays_status'] = ['in', '1,2'];
            $stat['todaysum'] = M('Order')->where($todaysumMap)->sum('pays_amount');

            //平台收入
            $pays_poundage = M('Order')->where($todaysumMap)->sum('pays_poundage');
            ...
            $stat['platform'] = $pays_poundage - $order_cost - $profitSum;
            //代理收入
            $stat['agentIncome'] = $profitSum;

            //本月成功交易总额
            $monthBegin = date('Y-m-01').' 00:00:00';
            $monthsumMap['pays_successdate'] = ['egt', strtotime($monthBegin)];
            $monthsumMap['pays_status'] = ['in', '1,2'];
            $stat['monthsum'] = M('Order')->where($monthsumMap)->sum('pays_amount');

            //本月平台收入
            $pays_poundage = M('Order')->where($monthsumMap)->sum('pays_poundage');
            $profitSumMap['datetime'] = ['egt', $monthBegin];
            $profitSumMap['lx'] = 9;
            $profitSum = M('moneychange')->where($profitSumMap)->sum('money');
            $order_cost = M('Order')->where($monthsumMap)->sum('cost');
            $stat['monthPlatform'] = $pays_poundage - $order_cost - $profitSum;
            //代理收入
            $stat['monthAgentIncome'] = $profitSum;

            if($status == 1) {
                $nopaidsumMap['pays_applydate'] = ['between', [strtotime($todayBegin), strtotime($todyEnd)]];
                $nopaidsumMap['pays_status'] = 1;
                //今日异常订单总额
                $stat['todaynopaidsum'] = M('Order')->where($nopaidsumMap)->sum('pays_amount');
                //今日异常订单笔数
                $stat['todaynopaidcount'] = M('Order')->where($nopaidsumMap)->count();

                $monthNopaidsumMap['pays_applydate'] = ['egt', strtotime($todayBegin)];
                $monthNopaidsumMap['pays_status'] = 1;
                //本月异常订单总额
                $stat['monthNopaidsum'] = M('Order')->where($monthNopaidsumMap)->sum('pays_amount');
                //本月异常订单笔数
            ...
            $todyEnd = date('Y-m-d').' 23:59:59';
            $monthBegin = date('Y-m-01').' 00:00:00';
            $stat['todaynopaidsum'] = M('Order')->where(['pays_applydate'=>['between', [strtotime($todayBegin), strtotime($todyEnd)]], 'pays_status'=>0])->sum('pays_amount');
            $stat['monthNopaidsum'] = M('Order')->where(['pays_applydate'=>['egt', strtotime($monthBegin)], 'pays_status'=>0])->sum('pays_amount');
            $nopaidMap = $where;
            $nopaidMap['pays_status'] = 0;
            $stat['totalnopaidsum'] = M('Order')->alias('as O')->where($nopaidMap)->sum('pays_amount');
        }
        foreach($stat as $k => $v) {
            $stat[$k] = $v+0;
    ...
    public function exportorder()
    {
        ...
        $tongdao = I("request.tongdao");
        if ($tongdao) {
            $where['pays_bankcode'] = array('eq', $tongdao);
        }
        $bank = I("request.bank", '', 'strip_tags');
        if ($bank) {
            $where['pays_bankname'] = array('eq', $bank);
        }
        $status = I("request.status",'');
        if ($status != "") {
            if ($status == '1or2') {
                $where['pays_status'] = array('between', array('1', '2'));
            } else {
                $where['pays_status'] = array('eq', $status);
            }
        }
        $createtime = urldecode(I("request.createtime"));
        if ($createtime) {
        ...
        $numberField = ['pays_amount', 'pays_poundage', 'pays_actualamount'];
        $data = M('Order')
            ->join('LEFT JOIN __MEMBER__ ON __MEMBER__.id+10000 = __ORDER__.pays_memberid')
            ->where($where) ->field('pays_order.*, pays_member.username')->select();
        foreach ($data as $item) {

            switch ($item['pays_status']) {
                case 0:
                    $status = '未处理';
                    break;
                case 1:
            ...
            $list[] = [
                ...
                'pays_successdate'  => $pays_successdate,
    ...
    public function show()
    {
        ...
        if ($id) {
            $order = M('Order')
                ->join('LEFT JOIN __MEMBER__ ON (__MEMBER__.id + 10000) = __ORDER__.pays_memberid')
                ->field('pays_member.id as userid,pays_member.username,pays_member.realname,pays_order.*')
                ->where(['pays_order.id' => $id])
                ->find();
        }
    ...
    public function delAll() {
        ...
        if(IS_POST) {
            ...
            if ($createtime) {
                ...
                $where['pays_applydate'] = ['between', [$startTime, $endTime]];
            } else {
    ...
    public function setOrderPaid() {
        ...
        if(IS_POST) {
            ...
            $res = $payModel->completeOrder($order['pays_orderid'], '', 0);
            if ($res) {
    ...
    public function doForzen(){
        ...
        $userId=(int)$order['pays_memberid']-10000;

        M()->startTrans();
        $order=M("order")->where(array("id"=>$orderId,"pays_status"=>['in','1,2'],"lock_status"=>['LT',1]))->lock(true)->find();
        $member=M('member')->where(array('id' => $userId))->find();

        //需要检测是否已解冻，如果未解冻直接修改自动解冻状态，如果解冻，直接扣余额
        $maps['status'] = array('eq',0);
        $maps['orderid']=array('eq',$order['pays_orderid']);
        $blockedLog = M('blockedlog')->where($maps)->find();
        if($blockedLog){
            $res=M('blockedlog')->where(array('id'=>$blockedLog['id']))->save(array('status'=>1));

        }else{
            $res        = M('member')->where(array('id' => $userId,'balance'=>array("EGT",$order['pays_actualamount'])))->save([
                'balance' => array('exp', "balance-".$order['pays_actualamount']),
                'blockedbalance' => array('exp', "blockedbalance+".$order['pays_actualamount']),
            ]);
        }

       $orderRe =M("order")->where(array("id"=>$orderId,"pays_status"=>['in','1,2'],"lock_status"=>['LT',1]))->save(['lock_status'=>1]);
        if($res!==false&&$orderRe!==false){
            ...
            $moneychange_data = [
                ...
                'money'      => $order['pays_actualamount'],
                'gmoney'     => $member['balance']-$order['pays_actualamount'], //改动后的金额或冻结资金
                'datetime'   => date('Y-m-d H:i:s'),
                'tongdao'    => $order['pays_bankcode'],
                'transid'    => $order['pays_orderid'],
    ...
    public function thawOrder(){
        ...
        $userId=$order['pays_memberid']-10000;
        M()->startTrans();
        $order=M("order")->where(array("id"=>$orderId,"pays_status"=>['in','1,2'],"lock_status"=>['eq',1]))->lock(true)->find();
                $member=M('member')->where(array('id' => $userId))->find();

        //需要检测是否已解冻，如果未解冻直接修改自动解冻状态，如果解冻，直接扣余额
         $res        = M('member')->where(array('id' => $userId,'blockedbalance'=>array('EGT',$order['pays_actualamount'])))->save([
                'balance' => array('exp', "balance+".$order['pays_actualamount']),
                'blockedbalance' => array('exp', "blockedbalance-".$order['pays_actualamount']),
            ]);
         //记录日志
         $orderRe=M("order")->where(array("id"=>$orderId,"pays_status"=>['in','1,2'],"lock_status"=>['eq',1]))->save(array("lock_status"=>2));
        if($res!==false&&$orderRe!==false){
            M()->commit();
            
             $moneychange_data = [
                'userid'     => $order['pays_memberid'],
                'ymoney'     => $member['balance'], //原金额或原冻结资金
                'money'      => $order['pays_actualamount'],
                'gmoney'     => $member['balance']+$order['pays_actualamount'], //改动后的金额或冻结资金
                'datetime'   => date('Y-m-d H:i:s'),
    ...
    public function frozenOrder(){
        ...
        $this->assign('bank', $bank);
        $payOrderid = I('get.pays_orderid', '');
        if ($payOrderid) {
            $where['O.pays_orderid'] = array('eq', $payOrderid);
        }
        $this->assign('pays_orderid', $payOrderid);
        $body = I("request.body", '', 'strip_tags');
        if ($body) {
            $where['O.pays_productname'] = array('eq', $body);
        }
