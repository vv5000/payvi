...
class PayForAnotherController extends BaseController{
	...
	public function extendFields(){
        ...
        $channel    = M('pays_for_another')->where(['id' => $channel_id])->find();
        $data   = M('pays_channel_extend_fields')->where(['channel_id' => $channel_id])->select();
    ...
    public function editExtendFields()
    {
        if(IS_POST) {
            ...
            $count = M('pays_channel_extend_fields')->where(array('name'=>$data['name'], 'channel_id' => $data['channel_id'], 'id'=>array('neq', $data['id'])))->count();
            if($count>0) {
                $this->ajaxReturn(['status' => 0, 'msg' => '该扩展字段名已存在']);
            }
            $res = M('pays_channel_extend_fields')->where(['id' => $data['id']])->save($data);
            if(FALSE !== $res) {
            ...
            $id = I('id', 0, 'intval');
            if ($id) {
                $data = M('pays_channel_extend_fields')->where(['id' => $id])->find();
            }
            $this->assign('data', $data);
    ...
    public function addExtendFields()
    {
        if(IS_POST) {
            ...
            $count = M('pays_channel_extend_fields')->where(array('name'=>$data['name'], 'channel_id' => $data['channel_id']))->count();
            if($count>0) {
                $this->ajaxReturn(['status' => 0, 'msg' => '该扩展字段名已存在']);
            }
            $data['code']  = M('pays_for_another')->where(array('id' => $data['channel_id']))->getField('code');
            $data['ctime'] = $data['etime'] = time();
            $res = M('pays_channel_extend_fields')->add($data);
            if(FALSE !== $res) {
    ...
    public function delExtendFields()
    {
        $id = I('id', 0, 'intval');
        if ($id) {
            $res = M('pays_channel_extend_fields')->where(['id' => $id])->delete();
            $this->ajaxReturn(['status' => $res]);
        }
    }
}
