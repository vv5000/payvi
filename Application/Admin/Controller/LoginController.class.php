...
class LoginController extends Controller
{
        ...
        if (IS_POST) {
            ...
            if ($info) {
				...
				if($siteconfig['google_auth'] && !session('google_auth')) {
					$this->ajaxReturn(['errorno' => 0, 'msg' => '登录成功，进行二次身份验证', 'url' => U('Login/google')]);
				} else {
	...
	public function statics(){
		...
		$t2      = I("t2", '', 'trim');
		$where    = array();
		$where['pays_applydate'] = ['between', [$t1, $t2]];
		$field = ['sum(`pays_amount`) total_amount','count(`id`) total_count'];
		$sum = M('Order')->field($field)->where($where)->find();
		$this->ajaxReturn(['count' => $sum['total_count'], 'amount' => $sum['total_amount']]);
	}
    ...
    {
        if(IS_POST) {
            $google_secret_key = M('admin')->where(['id'=>session('admin_auth')['uid']])->getField('google_secret_key');
            if($google_secret_key == '') {
                $this->error("您未绑定谷歌身份验证器");
            }

            if(!session('admin_auth')) {
                $this->ajaxReturn(['status' => 0, 'msg' => '密码验证不通过！']);
            }
