...
class StatisticsController extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($memberid) {
            $where['O.pays_memberid'] = array('eq', $memberid);
            $profitMap['userid'] = $profitSumMap['userid']= $memberid-10000;
        }
        $orderid = I("request.orderid");
        if ($orderid) {
            $where['O.pays_orderid'] = $orderid;
            $profitMap['transid'] = $orderid;
        }
        $tongdao = I("request.tongdao");
        if ($tongdao) {
            $where['O.pays_tongdao'] = array('eq', $tongdao);
        }

        $createtime = urldecode(I("request.createtime"));
        if ($createtime) {
            list($cstime, $cetime)  = explode('|', $createtime);
            $where['O.pays_applydate'] = ['between', [strtotime($cstime), strtotime($cetime) ? strtotime($cetime) : time()]];
        ...
        if ($successtime) {
            ...
            $where['O.pays_successdate'] = ['between', [strtotime($sstime), strtotime($setime) ? strtotime($setime) : time()]];
            $profitMap['datetime'] = ['between', [$cstime, $cetime ? $cetime : date('Y-m-d H:i:s')]];
        } else if (!$successtime && !$createtime) {
            $_GET['successtime']      = date('Y-m-d H:i:s', strtotime(date('Y-m', time()))) . " | " . date('Y-m-d H:i:s', time());
            $where['O.pays_successdate'] = ['between', [strtotime(date('Y-m', time())), time()]];
        ...
        foreach ($list as $item) {
            if ($item['pays_status'] >= 1) {
                $amount += $item['pays_amount'];
                $rate += $item['pays_poundage'];
                $realmoney += $item['pays_actualamount'];
            }
        }
        //查询支付成功的订单的手续费，入金费，总额总和
        $countWhere               = $where;
        $countWhere['O.pays_status'] = ['between', [1, 2]];
        $field                    = ['sum(`pays_amount`) pays_amount','sum(`cost`) cost', 'sum(`pays_poundage`) pays_poundage', 'sum(`pays_actualamount`) pays_actualamount', 'count(`id`) success_count'];
        $sum                      = M('Order')->alias('as O')->field($field)->where($countWhere)->find();
        foreach ($sum as $k => $v) {
            $sum[$k] += 0;
        }
        $countWhere['O.pays_status'] = 0;
        //失败笔数
        $sum['fail_count'] =  M('Order')->alias('as O')->where($countWhere)->count();
        //投诉保证金冻结金额
        $map = $where;
        $map['C.status'] = 0;
        $sum['complaints_deposit_freezed'] = M('complaints_deposit')->alias('as C')->join('LEFT JOIN __ORDER__ AS O ON C.pays_orderid=O.pays_orderid')
            ->where($map)
            ->sum('freeze_money');
        $sum['complaints_deposit_freezed'] += 0;
        $map['C.status'] = 1;
        $sum['complaints_deposit_unfreezed'] = M('complaints_deposit')->alias('as C')->join('LEFT JOIN __ORDER__ AS O ON C.pays_orderid=O.pays_orderid')
            ->where($map)
            ->sum('freeze_money');
       ...
       $sum['memberprofit1'] = M('moneychange')->where(['lx'=>9])->sum('money'); //修复bug 原bug不显示代理总分成
        ...
        $sum['pays_poundage1'] = $sum['pays_poundage']- $sum['cost'];

        $sum['memberprofit'] = M('moneychange')->where(['lx'=>9])->sum('money');
        $sum['pays_poundage'] = $sum['pays_poundage'] - $sum['cost'] - $sum['memberprofit1'];
        foreach($sum as $k => $v) {
            $sum[$k] +=0;
        }
        //统计订单信息
        ...
        $time       = $successtime ? 'pays_successdate' : 'pays_applydate';
        $dateFormat = $is_month ? '%Y年-%m月' : '%Y年-%m月-%d日';
        $field      = "FROM_UNIXTIME(" . $time . ",'" . $dateFormat . "') AS date,SUM(pays_amount) AS amount,SUM(pays_poundage) AS rate,SUM(pays_actualamount) AS total";
        ...
        $this->assign('stamount', $sum['pays_amount']);
        $this->assign('stamount1', $sum['pays_amount']);//新总入金
        $this->assign('strate', $sum['pays_poundage']);
        $this->assign('strate1', $sum['pays_poundage1']); //新平台总分成
        $this->assign('strealmoney', $sum['pays_actualamount']);
    ...
    public function exportorder()
    {
        ...
        $memberid = I("request.memberid");
        if ($memberid) {
            $where['pays_memberid'] = array('eq', $memberid);
        }
        $orderid = I("request.orderid");
        if ($orderid) {
        ...
        $tongdao = I("request.tongdao");
        if ($tongdao) {
            $where['pays_tongdao'] = array('eq', $tongdao);
        }

        $createtime = urldecode(I("request.createtime"));
        if ($createtime) {
            list($cstime, $cetime)  = explode('|', $createtime);
            $where['pays_applydate'] = ['between', [strtotime($cstime), strtotime($cetime) ? strtotime($cetime) : time()]];
        }
        $successtime = urldecode(I("request.successtime"));
        if ($successtime) {
            list($sstime, $setime)    = explode('|', $successtime);
            $where['pays_successdate'] = ['between', [strtotime($sstime), strtotime($setime) ? strtotime($setime) : time()]];
        }
        ...
        foreach ($data as $item) {
            $list[] = array(
                'pays_orderid'      => $item['pays_orderid'],
                'pays_memberid'     => $item['pays_memberid'],
                'pays_amount'       => $item['pays_amount'],
                'pays_poundage'     => $item['pays_poundage'],
                'pays_actualamount' => $item['pays_actualamount'],
                'pays_applydate'    => date('Y-m-d H:i:s', $item['pays_applydate']),
                'pays_successdate'  => date('Y-m-d H:i:s', $item['pays_successdate']),
                'pays_zh_tongdao'   => $item['pays_zh_tongdao'],
                'pays_status'       => '成功，已返回',
            );
        }
    ...
    public function userFinance()
    {
        ...
        if ($memberList) {
            ...
            foreach ($memberList as $k => $v) {
                ...
                $orderList = $Order
                    ->field(['sum(pays_amount) pays_amount', 'sum(pays_poundage) pays_poundage', 'sum(pays_actualamount) pays_actualamount'])
                    ->where(['pays_memberid' => $payMemberid, 'pays_status' => ['neq', 0]])
                    ->find();
                if (empty($orderList)) {
                    $orderList = ['pays_amount' => 0.00, 'pays_poundage' => 0.00, 'pays_actualamount' => 0.00];
                } else {
                    $orderList['pays_amount']+=0;
                    $orderList['pays_poundage']+=0;
                    $orderList['pays_actualamount']+=0;
                }
                ...
                $memberList[$k]['pays_memberid'] = $payMemberid;
                //提交订单数
                $memberList[$k]['all_order_count'] =  $Order->where(['pays_memberid' => $payMemberid])->count();
                $memberList[$k]['all_order_count'] +=0;
                //已付订单数
                $memberList[$k]['paid_order_count'] =  $Order->where(['pays_memberid' => $payMemberid, 'pays_status'=>['in', '1,2']])->count();
                $memberList[$k]['paid_order_count'] +=0;
                //未付订单数
                $memberList[$k]['nopaid_order_count'] = $Order->where(['pays_memberid' => $payMemberid, 'pays_status'=>0])->count();
                $memberList[$k]['nopaid_order_count'] +=0;
                //提交金额
                $memberList[$k]['all_order_amount'] = $Order->where(['pays_memberid' => $payMemberid])->sum('pays_amount');
                $memberList[$k]['all_order_amount'] +=0;
                //实付金额
                $memberList[$k]['paid_order_amount'] = $Order->where(['pays_memberid' => $payMemberid, 'pays_status'=>['in', '1,2']])->sum('pays_amount');
                $memberList[$k]['paid_order_amount'] +=0;
                //入金手续费
                $memberList[$k]['pays_amount'] = $Order->where(['pays_memberid' => $payMemberid, 'pays_status'=>['in', '1,2']])->sum('pays_poundage');
                $memberList[$k]['pays_amount'] +=0;
                //商户收入
                $actualamount = M('Order')->where(['pays_memberid'=>$payMemberid, 'status' => ['in', '1,2']])->sum('pays_actualamount');
                ...
                //平台收入
                $income_profit = M('Order')->where(['pays_memberid'=>$payMemberid, 'status' => ['in', '1,2']])->sum('pays_poundage');
                $order_cost = M('Order')->where(['pays_memberid'=>$payMemberid, 'status' => ['in', '1,2']])->sum('cost');
                $pays_cost = M('wttklist')->where(['userid'=>$v['id'], 'status' => 2])->sum('cost');
                $agent_profit_cost = M('moneychange')->where(['tcuserid'=>$v['id'],'lx'=>9])->sum('money');
                $memberList[$k]['platform_income'] = $income_profit - $order_cost - $pays_cost - $agent_profit_cost;
                $memberList[$k]['platform_income'] += 0;
            }
        }
        $stat['member_count'] = count($memberList);
        $fields = ['tkmoney', 'sxfmoney', 'money' , 'all_order_count', 'paid_order_count', 'nopaid_order_count', 'all_order_amount', 'paid_order_amount', 'member_income', 'platform_income','balance','blockedbalance','pays_amount','tkmoney','sxfmoney'];
    ...
    public function exportUserFinance(){
        ...
        foreach ($memberList as $m => $n){
            ...
            $list[] = array(
                ...
                'pays_poundage'=>$n['pays_poundage'],
                'balance'=>$n['balance'],
                'blockedbalance'=>$n['blockedbalance'],
                'pays_amount'=>$n['pays_amount'],
                'tkmoney'=>$n['tkmoney'],
                'sxfmoney'=>$n['sxfmoney'],
                'money'=>$n['money'],
        ...
        $list[] = array(
            ...
            'pays_poundage'=>$stat['paid_order_amount'].'元',
            'balance'=>$stat['balance'].'元',
            'blockedbalance'=>$stat['blockedbalance'].'元',
            'pays_amount'=>$stat['pays_amount'].'元',
            ...
            foreach ($orderList as $k1 => $v1) {
                if ($v['id'] == $v1['pays_bankcode']) {
                    ...
                    if ($v1['pays_status'] != 0) {
                        ...
                        $productList[$k]['pays_amount']       = bcadd($productList[$k]['pays_amount'], $v1['pays_amount'], 4);
                        $productList[$k]['pays_poundage']     = bcadd($productList[$k]['pays_poundage'], $v1['pays_poundage'], 4);
                        $productList[$k]['pays_actualamount'] = bcadd($productList[$k]['pays_actualamount'], $v1['pays_actualamount'], 4);
                    }
                }
            }
    ...
    public function productChannelFinance()
    {
        ...
        foreach($list as $k => $v) {
            $sum = M('Order')->field(['sum(`pays_amount`) pays_amount','sum(`cost`) cost', 'sum(`pays_poundage`) pays_poundage', 'sum(`pays_actualamount`) pays_actualamount', 'count(`id`) success_count'])
                ->where(['channel_id'=> $v['id'], 'pays_status' => ['in', '1,2']])->find();
            $list[$k]['pays_amount'] = $sum['pays_amount'];//交易笔数
            $list[$k]['pays_amount'] += 0;
            $list[$k]['pays_poundage'] = $sum['pays_poundage'];//手续费
            $list[$k]['pays_poundage'] += 0;
            $list[$k]['pays_actualamount'] = $sum['pays_actualamount'];//入金总额
            $list[$k]['pays_actualamount'] += 0;
    ...
    public function channelAccountFinance()
    {
        ...
        foreach($list as $k => $v) {
            $sum = M('Order')->field(['sum(`pays_amount`) pays_amount','sum(`cost`) cost', 'sum(`pays_poundage`) pays_poundage', 'sum(`pays_actualamount`) pays_actualamount', 'count(`id`) success_count'])
                ->where(['account_id'=> $v['id'], 'pays_status' => ['in', '1,2']])->find();
            $list[$k]['pays_amount'] = $sum['pays_amount'];//交易笔数
            $list[$k]['pays_amount'] += 0;
            $list[$k]['pays_poundage'] = $sum['pays_poundage'];//手续费
            $list[$k]['pays_poundage'] += 0;
    ...
    //     //-----------订单表-------------
    //     $orderWhere = ['pays_memberid' => ['in', $memberids], 'pays_status' => ['between', [1, 2]]];
    ...
    //         $orderWhere['pays_successdate'] = ['between', $orderTime];
    //     }

    //     $Order      = M('Order');
    //     $orderLists = $Order->where($orderWhere)->select();
    //     $orderField = ['sum(`pays_amount`) amount, sum(`pays_poundage`) poundage, sum(`pays_actualamount`) actualamount, sum(`cost`) cost'];
    //     $orderSum   = $Order->field($orderField)->where(['pays_status' => ['between', [1, 2]]])->find();
    ...
    //             'money'            => 0, //代付+提现 实际金额
    //             'pays_amount'       => 0, //订单的金额
    //             'pays_poundage'     => 0, //订单的手续费
    //             'pays_actualamount' => 0, //订单入金总额
    //             'order_cost'       => 0, //成本费
    //             'wl_cost'          => 0, //代付成本费
    //             'lx1'              => 0, //用户的入金总额
    //             'lx3'              => 0, //手动增加的
    ...
    //             if ($v1['pays_memberid'] == $memberid) {
    //                 $sum['pays_amount']       = bcadd($v1['pays_amount'], $sum['pays_amount'], 4);
    //                 $sum['pays_poundage']     = bcadd($v1['pays_poundage'], $sum['pays_poundage'], 4);
    //                 $sum['pays_actualamount'] = bcadd($v1['pays_actualamount'], $sum['pays_actualamount'], 4);
    ...
    //         $sum['all_poundage'] = bcadd($sum['sxfmoney'], $sum['pays_poundage'], 4);
    ...
    //             'sum(`pays_poundage`) poundage',
    //             'sum(`pays_actualamount`) actualamount',
    //             'sum(`cost`) cost',
    //             'pays_zh_tongdao',
    //             'pays_tongdao',
    //         ];

    //         $orderWhere = ['pays_memberid' => $memberid, 'pays_status' => ['between', [1, 2]]];
    //         $orderTime  = I('request.order_time', '');
    //         if ($orderTime) {
    //             $orderTime                     = explode('|', $orderTime);
    //             $orderTime[0]                  = strtotime($orderTime[0]);
    ...
    //         }
    //         //获取总的订单数据
    //         $orderLists = $Order->field($orderField)->where($orderWhere)->group('pays_tongdao')->select();
    ...
    //             'pays_amount'       => $v['pays_amount'],
    //             'pays_actualamount' => $v['pays_actualamount'],
    ...
    public function chargeRank() {
        ...
        $count = M()->query("SELECT count('a.*') as count FROM(SELECT pays_member.id as userid FROM `pays_order` LEFT JOIN pays_member ON (pays_member.id + 10000) = pays_order.pays_memberid WHERE `pays_successdate` BETWEEN ".strtotime($cstime)." AND ".strtotime($cetime)." AND `pays_status` > 0 AND `ddlx` = 1 GROUP BY pays_memberid)  a");
        $count = $count[0]['count'];
        $size = 50;
        $rows = I('get.rows', $size, 'intval');
        if (!$rows) {
        ...
        $where['pays_successdate'] = ['between', [strtotime($cstime), strtotime($cetime) ? strtotime($cetime) : time()]];
        $where['pays_status'] = ['gt',0];
        //$where['ddlx'] = 1;
        $page = new Page($count, $rows);
        $list = M('Order')
            ->join('LEFT JOIN __MEMBER__ ON (__MEMBER__.id + 10000) = __ORDER__.pays_memberid')
            ->field('pays_member.id as userid,pays_member.username,pays_member.realname,sum(pays_amount) as total_charge')
            ->where($where)
            ->limit($page->firstRow . ',' . $page->listRows)
            ->group('pays_memberid')
    ...
    public function platformReport() {
        ...
        if ($data) {
            foreach ($data as $k => $v) {
                ...
                //入金金额
                $orderSum = M('Order')->where(['pays_memberid'=>10000+$v['id'],'pays_successdate' => ['between', [$begin, $end]], 'pays_status' => ['in', '1,2']])->sum('pays_amount');
                ...
                $data[$k]['pays_wait_checked'] = $payWaitCheckedSum1 + $payWaitCheckedSum2;
                //出金成功金额
                ...
                $data[$k]['pays_success'] = $payCheckedSum1 + $payCheckedSum2;
                //出金失败金额
                ...
                $data[$k]['pays_fail'] = $payFailSum1 + $payFailSum2;
                //入金利润
                $actualamount = M('Order')->where(['pays_memberid'=>10000+$v['id'],'pays_successdate' => ['between', [$begin, $end]], 'pays_status' => ['in', '1,2']])->sum('pays_actualamount');
                ...
                $data[$k]['pays_profit'] = $tkmoney1 + $tkmoney2 - $money1 - $money2 + $data[$k]['cjsxf'];
                //冻结金额
                ...
                //平台成本
                $cost1 = M('Order')->where(['pays_memberid'=>10000+$v['id'],'pays_successdate' => ['between', [$begin, $end]], 'pays_status' => ['in', '1,2']])->sum('cost');
                ...
                //平台利润
                $data[$k]['platform_profit'] = $data[$k]['income_profit'] +  $data[$k]['pays_profit'] - $data[$k]['platform_cost'] + $data['cjsxf'];
                //期末余额
        ...
        if ($data) {
            foreach ($data as $k => $v) {
                ...
                $data[$k]['channel_rate'] = M('ProductUser')
                    ...
                    ->where(['pays_product_user.userid'=>$v['id'],'pays_product_user.status'=>1,'pays_product.isdisplay'=>1])
                    ->field('pays_product.name,pays_product.id,pays_product.code,pays_product_user.status')
                    ->select();
                ...
                $data[$k]['frozen_money'] = $frozen_money1 + $frozen_money2 + $frozen_money3;
                //商户实际到账金额
                $actualamount = M('Order')->where(['pays_memberid'=>10000+$v['id'],'pays_successdate' => ['between', [$begin, $end]], 'status' => ['in', '1,2']])->sum('pays_actualamount');
                $orderSum = $actualamount + $profitSum + $redoAddSum - $redoReduceSum;
   ...
   */
    ...
    public function exportPlatform(){
        ...
        if ($data) {
            foreach ($data as $k => $v) {
                ...
                $data[$k]['platform_cost'] = $cost1 + $cost2 + $cost3;
                //平台利润
                $data[$k]['platform_profit'] = $data[$k]['income_profit'] +  $data[$k]['pays_profit'] - $data[$k]['platform_cost'];
                //期末余额
                $data[$k]['end_profit'] = $this->getDateBalance($v['id'],$endDate);
                //当前余额
                $data[$k]['current_money'] = $v['balance'];
            ...
            foreach ($data as $m => $n){
                $list[] = array(
                    ...
                    'pays_wait_checked'=>$n['pays_wait_checked'],
                    'pays_success'=>$n['pays_success'],
                    'pays_fail'=>$n['pays_fail'],
                    'cjsxf'=>$n['cjsxf'],
                    'income_profit'=>$n['income_profit'],
                    'pays_profit'=>$n['pays_profit'],
    ...
    public function getPlatformProfit() {
        ...
        //入金金额
        $orderSum = M('Order')->where(['pays_status' => ['in', '1,2']])->sum('pays_amount');
        $data['income_money'] = $orderSum + $redoAddSum - $redoReduceSum;

        //入金利润
        $actualamount = M('Order')->where(['pays_status' => ['in', '1,2']])->sum('pays_actualamount');
        ...
        $data['pays_profit'] = $tkmoney1 + $tkmoney2 - $money1 - $money2;
        //平台成本
        $cost1 = M('Order')->where(['pays_status' => ['in', '1,2']])->sum('cost');
        $cost2 = M('wttklist')->where([ 'status' => 2])->sum('cost');
        $cost3 = M('moneychange')->where(['lx'=>9])->sum('money');
        $data['platform_cost'] = $cost1 + $cost2 + $cost3;
        //平台利润
        $platform_profit = $data['income_profit'] +  $data['pays_profit'] - $data['platform_cost'];
