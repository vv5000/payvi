...
class PayModel
{
     ...
     */
    public function completeOrder($trans_id, $pays_name = '', $returntype = 1, $transaction_id = '')
    {
    ...
    protected function saveOfflineStatus($model, $id, $pays_amount, $info)
    {
        if ($info['offline_status'] && $info['control_status'] && $info['all_money'] > 0) {
            //通道是否开启风控和支付状态为上线
            $data['paying_money']     = bcadd($info['paying_money'], $pays_amount, 2);
    ...
    private function bianliticheng($arrayStr, $num = 3, $tcjb = 1)
    {
        ...
        if ($feilvfind["status"] == "error") {
            ...
            $parentid = intval($parentid) - 10000;
            if ($parentid <= 1) {
                return false;
            }
