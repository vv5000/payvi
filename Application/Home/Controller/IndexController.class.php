...
class IndexController extends BaseController
{
    ...
    public function test2() {
        $map['pays_status'] = ['in','1,2'];
        //$map['pays_successdate'] = ['between',[1530460800,1530547199]];
        $list = M('Order')->where($map)->select();
		$count = 0;
        foreach($list as $k => $v) {
            $profit = $v['pays_poundage'] - $v['cost'];
            $yj = M('moneychange')->where(['lx'=>9, 'transid'=>$v['pays_orderid']])->sum('money');
            if($profit<0) {
				$count++;
                echo $v['pays_orderid'].'<br>';
            }
        }
        echo 'ok';die;
    }
public function test3(){
  ...
  */
    ...
    public function test4() {
        $map['pays_status'] = ['in','1,2'];
        $map['pays_memberid'] = 10020;
        $list = M('Order')->where($map)->select();
        foreach($list as $k => $v) {
            $log = M('Moneychange')->where(['lx'=>1,'userid'=>20,'transid'=>$v['pays_orderid']])->find();
            if(empty($log)) {
                echo '异常流水：'.$v['pays_orderid'].'<br>';
            } else {
                if($log['money'] != $v['pays_actualamount']) {
                    echo '金额异常：'.$v[pays_orderid].', 订单金额：'.$v['pays_actualamount'].',流水金额：'.$log['money'].'<br>';
                }
            }
        }
        echo 'compeleted';
