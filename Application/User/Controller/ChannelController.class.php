...
class ChannelController extends UserController
{
    ...
    public function index()
    {
        ...
        $list = M('ProductUser')
            ...
            ->where(['pays_product_user.userid'=>$this->fans['uid'],'pays_product_user.status'=>1,'pays_product.isdisplay'=>1])
            ->field('pays_product.name,pays_product.id,pays_product_user.status')
            ->select();
