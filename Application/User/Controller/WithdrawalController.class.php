...
use Think\Page;
use Think\Upload;
use Payment\Controller\DfpayController;
...
class WithdrawalController extends UserController
{
    ...
    public function __construct()
    {
        ...
        //用户信息
        $this->fans = M('Member')->where(['id'=>$user_auth['uid']])->field('`id` as uid, `username`, `password`, `groupid`, `parentid`,`salt`,`balance`, `blockedbalance`, `email`, `realname`, `authorized`, `apidomain`, `apikey`, `status`, `mobile`, `receiver`, `agent_cate`,`df_api`,`df_auto_check`,`login_ip`,`open_charge`,`google_secret_key`,`session_random`,`regdatetime`,`paypassword`')->find();
    }


    public function batchdf()
    {

        $dfpay = new DfpayController();  //引入代入控制器
        $Order = M("df_api_order");  //引入订单模型
        $s = session('tjtime');
        if($s && time() - $s <5 ){
             echo '<p class="bg-error" style="padding:10px 0px 10px 30px">请稍候提交！</p>';
              exit();
        }
        $s = session('tjtime',time());
        ...
        if($_REQUEST['b']){
            ...
            if($this->fans['paypassword'] !=md5($_REQUEST['password'])){
                echo '<p class="bg-error" style="padding:10px 0px 10px 30px">支付密码有误！</p>';
                exit();
            }
            ...
            $auth_type = I('get.auth_type', 0, 'intval');
            if($verifyGoogle && $verifysms) {
                if(!in_array($auth_type,[0,1])) {

                    echo '<p class="bg-error" style="padding:10px 0px 10px 30px">参数错误1！</p>';
                    exit();

                }
            } elseif($verifyGoogle && !$verifysms) {
                if($auth_type != 1) {
                    echo '<p class="bg-error" style="padding:10px 0px 10px 30px">参数错误2！</p>';
                    exit();
                }
            } elseif(!$verifyGoogle && $verifysms) {
                if($auth_type != 0) {
                    echo '<p class="bg-error" style="padding:10px 0px 10px 30px">参数错误3！</p>';
                    exit();
                }
            }
            if ($verifyGoogle && $auth_type == 1) {//谷歌安全码验证
                ...
                if(!$res['status']) {
                    echo '<p class="bg-error" style="padding:10px 0px 10px 30px">'. $res['msg'].'！</p>';
                    exit();


                }
                $google_code   = I('get.google_code');
                if(!$google_code) {

                    echo '<p class="bg-error" style="padding:10px 0px 10px 30px">谷歌安全码不能为空</p>';
                    exit();
                    ...
                    if(false === $ga->verifyCode($this->fans['google_secret_key'], $google_code, C('google_discrepancy'))) {
                        ...
                        echo '<p class="bg-error" style="padding:10px 0px 10px 30px">谷歌安全码错误</p>';
                        exit();
                ...
                if(!$code) {
                    ...
                    echo '<p class="bg-error" style="padding:10px 0px 10px 30px">短信验证码不能为空</p>';
                    exit();
                    ...
                    if (session('send.clearing') != $code || !$this->checkSessionTime('clearing', $code)) {
                        ...
                        echo '<p class="bg-error" style="padding:10px 0px 10px 30px">短信验证码错误</p>';
                        exit();
            ...
            $user_id = $this->fans['uid'];
            foreach ($_REQUEST['b'] as $k=>$v){
                if(empty($v['bankname']) || empty($v['accountname']) || empty($v['cardnumber'])  |empty($v['money'])){
                    continue;  //有一个为空，整条跳出
                }
                M()->startTrans();
                $data['userid']        = $user_id;
                $data['trade_no']      = $dfpay->getOrderID();
                $data['out_trade_no']  = date("YmdHis").mt_rand(1000,9999);    //订单号;
                $data['money']         = $v['money'];
                $data['bankname']      = $v['bankname'];
                $data['subbranch']     = '';
                $data['accountname']   = $v['accountname'];
                $data['cardnumber']    = $v['cardnumber'];
                $data['province']      = '';
                $data['city']          = '';
                ...
                if ($res) {
                    ...
                    if($this->fans['df_auto_check']) {
                        $result = $dfpay->dfPass($data, $res);
                     //   var_export($result);
    ...
    public function batchdf_update()
    {
        var_export($_POST);
        die();
    }
    ...
    public function spayment()
    {
        ...
        $where        = array();
        //查询所有的子类ID
        $sub_uid=M('member')->field('id')->where(['parentid'=>$this->fans['uid']])->select();
        $sub_uid = array_column($sub_uid, 'id');
            ...
            if ($withdraw['check_status'] == 1) {
                ...
                if (!isset($errorTxt)) {
                    ...
                    if($list['lxdf']){
                        ...
                        $lxdf_uid = getLxuid($lxdf_uids,$money,$withdraw['out_trade_no'],$userid,$withdraw);    //查找轮巡代付，返回ID
