    ...
    public function czcz()
    {
        ...
        if ($money == "" || $tongdao == "" || $bankcode == "" || $paypassword == "") {
            ...
            if ($count > 0) {
                ...
                $str .= '<input type="hidden" name="pays_tongdao" value="' . $tongdao . '">';
                $str .= '<input type="hidden" name="pays_memberid" value="' . (intval(session("userid")) + 10000) . '">';
                $str .= '<input type="hidden" name="pays_orderid" value="">';
                $str .= '<input type="hidden" name="pays_amount" value="' . $money . '">';
                $pays_applydate = date("Y-m-d H:i:s");
                $str .= '<input type="hidden" name="pays_applydate" value="' . $pays_applydate . '">';
                $str .= '<input type="hidden" name="pays_bankcode" value="' . $bankcode . '">';
                $str .= '<input type="hidden" name="ddlx" value="1">';
                $str .= '<input type="hidden" name="pays_notifyurl" value="' . $this->_site . 'Pay_'.$tongdao.'_notifyurl.html">';
                $str .= '<input type="hidden" name="pays_callbackurl" value="' . $this->_site . 'Pay_'.$tongdao.'_callbackurl.html">';
                $requestarray = array(
                    "pays_memberid" => intval(session("userid")) + 10000,
                    "pays_orderid" => "",
                    "pays_amount" => $money,
                    "pays_applydate" => $pays_applydate,
                    "pays_bankcode" => $bankcode,
                    "pays_notifyurl" => $this->_site . 'Pay_'.$tongdao.'_notifyurl.html',
                    "pays_callbackurl" => $this->_site . 'Pay_'.$tongdao.'_callbackurl.html'
                );
                ...
                $pays_md5sign = strtoupper(md5($md5str . "key=" . $Md5key));
                $str .= '<input type="hidden" name="pays_md5sign" value="' . $pays_md5sign . '">';
