...
class AgentController extends UserController
{
    ...
    public function childord()
    {
        ...
        $where = array('pays_memberid' => $userid);
        //商户号
        $memberid = I("request.memberid");
        if ($memberid) {
            $where['pays_memberid'] = $memberid;
        }
        //提交时间
        $createtime = urldecode(I("request.createtime"));
        if ($createtime) {
            list($cstime, $cetime)  = explode('|', $createtime);
            $where['pays_applydate'] = $poundageMap['datetime'] = ['between', [strtotime($cstime), strtotime($cetime) ? strtotime($cetime) : time()]];
        }
        //成功时间
        $successtime = urldecode(I("request.successtime"));
        if ($successtime) {
            list($sstime, $setime)    = explode('|', $successtime);
            $where['pays_successdate'] = $poundageMap['datetime'] = ['between', [strtotime($sstime), strtotime($setime) ? strtotime($setime) : time()]];
        }
        //查询下级数据
        $where['pays_status'] = ['in', '0,1,2'];
        $statistic = M('Order')->field(['sum(`pays_amount`) pays_amount, sum(`pays_poundage`) pays_poundage, sum(`pays_actualamount`) pays_actualamount'])->where($where)->find();
        //代理分润
        $poundageMap['tcuserid'] = $userid - 10000;
        $poundageMap['userid'] = $this->fans['uid'];
        $poundageMap['lx'] = 9;
        $pays_poundage = M('moneychange')->where($poundageMap)->sum('money');
        $this->assign('pays_amount', number_format($statistic['pays_amount'], 2));
        $this->assign('pays_poundage', number_format($pays_poundage, 2));
        $this->assign('pays_actualamount', number_format($statistic['pays_actualamount'], 2));

        //分页
        $count = M('Order')->where($where)->count();
        $Page  = new Page($count, 10);
        $data  = M('Order')->join('LEFT JOIN __MEMBER__ ON __MEMBER__.id+10000 = __ORDER__.pays_memberid')->where($where)->field('pays_order.*, pays_member.username')->limit($Page->firstRow . ',' . $Page->listRows)->order(['id' => 'desc'])->select();
        $show  = $Page->show();
    ...
    public function order()
    {
        ...
        if ($createtime) {
            ...
            $where['pays_applydate']      = ["between", [strtotime($starttime), strtotime($endtime)]];
        }
        $this->assign('createtime', $createtime);
        if ($successtime) {
            list($starttime, $endtime) = explode('|', $successtime);
            $where['pays_successdate']     = ["between", [strtotime($starttime), strtotime($endtime)]];
        }
        ...
        */
        $where['pays_status'] = array('in','0,1,2');
        $pays_memberid = [];
        ...
        if($user_id) {
            foreach($user_id as $k => $v) {
                array_push($pays_memberid, $v+10000);
            }
            if(!$createtime and !$successtime) {
                //今日成功交易总额
                $todayBegin = date('Y-m-d').' 00:00:00';
                $todyEnd = date('Y-m-d').' 23:59:59';
                $stat['todaysum'] = M('Order')->where(['pays_memberid'=>['in', $pays_memberid],'pays_successdate'=>['between', [strtotime($todayBegin), strtotime($todyEnd)]], 'pays_status'=>['in', '1,2']])->sum('pays_amount');
                //今日成功笔数
                $stat['todaysuccesscount'] = M('Order')->where(['pays_memberid'=>['in', $pays_memberid],'pays_successdate'=>['between', [strtotime($todayBegin), strtotime($todyEnd)]], 'pays_status'=>['in', '1,2']])->count();
                //总成功交易总额
                $totalMap['pays_memberid'] = ['in', $pays_memberid];
                $totalMap['pays_status'] = ['in', '1,2'];
                $stat['totalsum'] = M('Order')->where($totalMap)->sum('pays_amount');
                //总成功笔数
            ...
            if($memberid) {
                if(in_array($memberid, $pays_memberid)) {
                    $where['pays_memberid'] = $memberid;
                } else {
                    $where['pays_memberid'] = 1;
                }
            } else {
                $where['pays_memberid'] = ['in', $pays_memberid];
            }
            //如果指定时间范围则按搜索条件做统计
    ...
    public function exportorder()
    {
        ...
        $where['pays_status'] = array('in','1,2');
        $pays_memberid = [];
        $user_id = M('Member')->where(['parentid'=>$this->fans['uid']])->getField('id', true);
        if($user_id) {
            foreach($user_id as $k => $v) {
