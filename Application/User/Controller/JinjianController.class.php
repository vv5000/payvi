		...
		if(IS_POST) {
			...
			if(FALSE !== $this->model->where(['id'=>$id])->save($data)) {
				...
				foreach($payment as $k => $v) {
					$pay = M('jinjian_pay')->where(array('pays_type'=>$k,'jid'=>$id))->find();
					if(empty($pay)) {
						$payData['pays_type'] = $k;
						$payData['pays_name'] = $paytype[$k];
						$payData['cycle'] = 'D1';
						$payData['rate'] = $v;
						$payData['jid'] = $id;
						...
						if($v != $pay['rate']) {
							M('jinjian_pay')->where(array('pays_type'=>$k,'jid'=>$id))->save(array('rate'=>$v, 'status'=>2));
						}
					}
				}
			...
			foreach($paylist as $k => $v) {
				$choosed[$v['pays_type']] = $v['rate'];
				if(in_array($v['pays_type'],array(1,2,3,4))) {
					$payTypeWechat = 1;
				}
				if(in_array($v['pays_type'],array(7,8,9))) {
					$payTypeAlipay= 1;
				}
			}
