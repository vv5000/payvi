...
class IndexController extends UserController
{
    ...
    public function main()
    {
        ...
        //成交金额
        $sql = "SELECT SUM( pays_actualamount ) AS total, FROM_UNIXTIME( pays_successdate,  '%Y-%m-%d' ) AS DATETIME
FROM pays_order WHERE pays_successdate >= UNIX_TIMESTAMP(  '".$firstday."' ) AND pays_successdate < UNIX_TIMESTAMP(  '".
            $lastday."' ) AND pays_status>=1 AND pays_memberid=".($this->fans['memberid'])."  GROUP BY DATETIME";
        $ordertotal = M('Order')->query($sql);

        //成交订单数
        $sql = "SELECT COUNT( id ) AS num, FROM_UNIXTIME( pays_successdate,  '%Y-%m-%d' ) AS DATETIME
FROM pays_order WHERE pays_successdate >= UNIX_TIMESTAMP(  '".$firstday."' ) AND pays_successdate < UNIX_TIMESTAMP(  '".
            $lastday."' ) AND pays_status>=1 AND pays_memberid=".($this->fans['memberid'])."  GROUP BY DATETIME";
        ...
        $stat['todayordercount'] = M('Order')->where(['pays_memberid'=>10000+$this->fans['uid'],'pays_applydate'=>['between', [strtotime($todayBegin), strtotime($todyEnd)]]])->count();
        //今日已付订单数
        $stat['todayorderpaidcount'] = M('Order')->where(['pays_memberid'=>10000+$this->fans['uid'],'pays_successdate'=>['between', [strtotime($todayBegin), strtotime($todyEnd)]], 'pays_status'=>['in', '1,2']])->count();
        //今日未付订单数
        $stat['todayordernopaidcount'] = M('Order')->where(['pays_memberid'=>10000+$this->fans['uid'],'pays_applydate'=>['between', [strtotime($todayBegin), strtotime($todyEnd)]], 'pays_status'=>0])->count();
        //今日提交金额
        //$stat['todayordersum'] = M('Order')->where(['pays_memberid'=>10000+$this->fans['uid'],'pays_applydate'=>['between', [strtotime($todayBegin), strtotime($todyEnd)]], 'pays_status'=>0])->sum('pays_amount');
        //今日实付金额
        $stat['todayorderactualsum'] = M('Order')->where(['pays_memberid'=>10000+$this->fans['uid'],'pays_successdate'=>['between', [strtotime($todayBegin), strtotime($todyEnd)]], 'pays_status'=>['in', '1,2']])->sum('pays_actualamount');
        //投诉保证金
