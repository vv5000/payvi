...
class RiskcontrolLogic extends \Think\Model
{
    ...
    protected function scopeOfAmount()
    {
        ...
        if ($min_money != 0.00 && $max_money != 0.00) {
            //判断用户交易的金额是否在交易范围
            if ($this->pays_amount < $min_money || $this->pays_amount > $max_money) {
                return '单笔交易金额[' . $min_money . '/' . $max_money . ']';
            }
        } else if ($min_money == 0.00 && $max_money != 0.00) {
            //判断用户交易的金额是否在交易范围
            if ($this->pays_amount > $max_money) {
                return '单笔交易最大金额[' . $max_money . ']';
            }
        } else if ($min_money != 0.00 && $max_money == 0.00) {
            //判断用户交易的金额是否在交易范围
            if ($this->pays_amount < $min_money) {
    ...
    protected function theTotalVolume(callable $callback)
    {
        if ($this->config_info['all_money'] != 0.00) {
            ...
            if ($now_date <= $last_paying_time) {
                ...
                $paying_money = $this->config_info['paying_money'] + $this->pays_amount;
    ...
    {   
        ...
        if ($this->config_info['unit_interval']) {
            ...
            if ($time_lag <= $unit_interval) {
                ...
                //判断时间间隔内交易次数
                $unit_pays_number = $this->config_info['unit_paying_number'] + 1;
                if ($unit_pays_number > $this->config_info['unit_number'] && $this->config_info['unit_number'] != 0) {
                    return '单位时间最大交易笔数/' . $this->config_info['unit_number'];
                }

                if ($this->config_info['unit_all_money'] != '0.00') {
                    //判断是否第一次交易且交易金额不能大于单位时间交易金额
                    if ($this->config_info['unit_frist_paying_time'] <= 0 && $this->pays_amount > $this->config_info['unit_all_money']) {
                        return '已超过单位时间总交易金额/' . $this->config_info['unit_all_money'];
                    }

                    //判断单位时间的交易金额
                    $unit_paying_amount = $this->config_info['unit_paying_amount'] + $this->pays_amount;
