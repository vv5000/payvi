...
class RepostController extends PayController
{
    ...
    public function postUrl()
    {
        ...
        $maps['pays_status'] = array('eq',1);
        $maps['num'] = array('lt',$nums);
        $maps['last_reissue_time'] = array('lt', time()-10);//距离上次补发至少10秒
        $list = M('Order')->where($maps)->field('id,pays_orderid,pays_ytongdao')->order('id asc')->limit(50)->select();
        if($list){
            foreach ($list as $item){
                $this->EditMoney($item['pays_orderid'],$item['pays_ytongdao'],0);
