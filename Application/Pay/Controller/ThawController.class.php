            ...
            if($data){
                foreach ($data as $item){
                    try{
                        ...
                        $Model = M();
                        $Model->startTrans();
                        $user = $Model->table('pays_member')->where(['id'=>$item['userid']])->lock(true)->find();
                        //更新资金
                        $upRes = $Model->table('pays_member')->where(['id'=>$item['userid'],'blockedbalance'=>['egt', $item['amount']]])->save($rows);
                        //更新状态
