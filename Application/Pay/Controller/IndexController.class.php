...
class IndexController extends PayController
{
    ...
    {
        $this->memberid = I("request.pays_memberid", 0, 'intval') - 10000;
        ...
        $this->pays_amount = I('post.pays_amount', 0);
        if ($this->pays_amount == 0) {
            $this->showmessage('金额不能为空');
        }

        //银行编码
        $this->bankcode = I('request.pays_bankcode', 0, 'intval');
        if ($this->bankcode == 0) {
            $this->showmessage('不存在的银行编码!', ['pays_banckcode' => $this->bankcode]);
        }

        $this->orderid = I('post.pays_orderid', '');
        if (!$this->orderid) {
