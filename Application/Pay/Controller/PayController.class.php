...
class PayController extends Controller
{
     ...
     * @return array
     */
    public function norderadd($parameter)
    {   

         $pays_amount = I("post.pays_amount", 0);
        ...
        $l_ChannelAccountRiskcontrol = new \Pay\Logic\ChannelAccountRiskcontrolLogic($pays_amount);
        ...
        $return = [
            ...
            "paytype"        => $syschannel["paytype"], // paytype
        ...
        if (!$pays_amount || !is_numeric($pays_amount) || $pays_amount <= 0) {
            $this->showmessage('金额错误@!');
        }
        $return["amount"] = floatval($pays_amount) * $moneyratio; // 交易金额
        $pays_sxfamount    = (($pays_amount * $feilv) > ($pays_amount * $fengding)) ? ($pays_amount * $fengding) :
        ($pays_amount * $feilv); // 手续费
        $pays_shijiamount = $pays_amount - $pays_sxfamount; // 实际到账金额
        if ($tikuanconfig['t1zt'] == 0) { //T+0费率
            $cost = bcmul($syschannel['t0rate'], $pays_amount, 2); //计算成本
        } else {
            $cost = bcmul($syschannel['rate'], $pays_amount, 2); //计算成本
        }
        ...
        //生成系统订单号
        $pays_orderid = $parameter['orderid'] ? $parameter['orderid'] : get_requestord();
        
            $Order                       = M("Order");
            $return['bankcode']          = $this->channel['pid'];
            $return['code']              = $platform['code']; //银行英文代码
            $return['orderid']           = $pays_orderid; // 系统订单号
            $return['out_trade_id']      = $out_trade_id; // 外部订单号
            $return['subject']           = $parameter['body']; // 商品标题
            $data['pays_memberid']        = $userid;
            $data['pays_orderid']         = $return["orderid"];
            $data['pays_amount']          = $pays_amount; // 交易金额
            $data['pays_poundage']        = $pays_sxfamount; // 手续费
            $data['pays_actualamount']    = $pays_shijiamount; // 到账金额
            $data['pays_applydate']       = time();
            $data['pays_bankcode']        = $this->channel['pid'];
            $data['pays_bankname']        = $platform['name'];
            $data['pays_notifyurl']       = I('post.pays_notifyurl', '');
            $data['pays_callbackurl']     = I('post.pays_callbackurl', '');
            $data['pays_status']          = 0;
            $data['pays_tongdao']         = $syschannel['code'];
            $data['pays_zh_tongdao']      = $syschannel['title'];
            $data['pays_channel_account'] = $syschannel['account'];
            $data['pays_ytongdao']        = $parameter["code"];
            $data['pays_yzh_tongdao']     = $parameter["title"];
            $data['pays_tjurl']           = $_SERVER['HTTP_REFERER'];
            $data['pays_productname']     = I("request.pays_productname");
            $data['attach']              = I("request.pays_attach");
         ...
         //  
            ...
            if ($Order->add($data)) {
                $return['datetime'] = date('Y-m-d H:i:s', $data['pays_applydate']);
     ...
     */
    protected function EditMoney($trans_id, $pays_name = '', $returntype = 1, $transaction_id = '')
    {

     
        $m_Order    = M("Order");
        $order_info = $m_Order->where(['pays_orderid' => $trans_id])->find(); //获取订单信息
        $userid     = intval($order_info["pays_memberid"] - 10000); // 商户ID
        $time       = time(); //当前时间

        //黑名单检测
        $this->check_black($order_info['channel_id']);

        //********************************************订单支付成功上游回调处理********************************************//
        if ($order_info["pays_status"] == 0 || $order_info["pays_status"] == 3) {
            ...
            {
            $res = $m_Order->where(['pays_orderid' => $trans_id, 'pays_status' => 0])
                ->save(['pays_status' => 1, 'pays_successdate' => $time]);
            }
            if($order_info["pays_status"] == 3)
            {
            $res = $m_Order->where(['pays_orderid' => $trans_id, 'pays_status' => 3])
                ->save(['pays_status' => 1, 'pays_successdate' => $time]);
            }
            if (!$res) {
                M()->rollback();
            ...
            //要给用户增加的实际金额（扣除投诉保证金）
            $actualAmount          = $order_info['pays_actualamount'];
            ...
            $moneychange_data = [
                ...
                'tongdao'    => $order_info['pays_bankcode'],
                'transid'    => $trans_id,
            ...
            $bianliticheng_data = [
                ...
                "money"   => $order_info["pays_amount"], // 金额
                "tongdao" => $order_info['pays_bankcode'],
            ];
            ...
            if ($channel_account_info['unit_interval']) {
                $m_ChannelAccount->where([
                    ...
                    'unit_paying_number' => ['exp', 'unit_paying_number+1'],
                    'unit_paying_amount' => ['exp', 'unit_paying_amount+' . $order_info['pays_actualamount']],
                ]);
            }
        ...
        $return_array = [ // 返回字段
            ...
            "amount"         => $order_info["pays_amount"], // 交易金额
        ...
        switch ($returntype) {
            case '0':
                ...
                curl_setopt($ch, CURLOPT_URL, $order_info["pays_notifyurl"]);
                curl_setopt($ch, CURLOPT_POSTFIELDS, $notifystr);
                $contents = curl_exec($ch);
                curl_close($ch);
                $httpCode = curl_getinfo($ch,CURLINFO_HTTP_CODE);
                log_server_notify($order_info["pays_orderid"], $order_info["pays_notifyurl"], $notifystr, $httpCode, $contents);
                ...
                if (strstr(strtolower($contents), "ok") != false) {
                    ...
                    $order_where = [
                        'id'          => $order_info['id'],
                        'pays_orderid' => $order_info["pays_orderid"],
                    ];
                    $order_result = $m_Order->where($order_where)->setField("pays_status", 2);
                } else {
                    // $this->jiankong($order_info['pays_orderid']);
                }
                break;
    ...
    //修改渠道跟账号风控状态
    protected function saveOfflineStatus($model, $id, $pays_amount, $info)
    {
        if ($info['offline_status'] && $info['control_status'] && $info['all_money'] > 0) {
            //通道是否开启风控和支付状态为上线
            $data['paying_money']     = bcadd($info['paying_money'], $pays_amount, 4);
    ...
    protected function verify()
    {
        //POST参数
        $requestarray = array(
            'pays_memberid'    => I('request.pays_memberid', 0, 'intval'),
            'pays_orderid'     => I('request.pays_orderid', ''),
            'pays_amount'      => I('request.pays_amount', ''),
            'pays_applydate'   => I('request.pays_applydate', ''),
            'pays_bankcode'    => I('request.pays_bankcode', ''),
            'pays_notifyurl'   => I('request.pays_notifyurl', ''),
            'pays_callbackurl' => I('request.pays_callbackurl', ''),
        );
    ...
    public function jiankong($orderid)
    {
        ...
        do {
            if ($orderid) {
                $_where['pays_status']  = 1;
                $_where['num']         = array('lt', 3);
                $_where['pays_orderid'] = $orderid;
                $find                  = $Order->where($_where)->find();
            } else {
                $find = $Order->where("pays_status = 1 and num < 3")->order("id desc")->find();
            }
            if ($find) {
                $this->EditMoney($find["pays_orderid"], $find["pays_tongdao"], 0);
    ...
    {
        $return = array();
        $order  = M('Order')->where(['pays_orderid' => $trans_id])->find();
        //用户费率
    ...
    public function bufa()
    {
        ...
        $m          = M("Order");
        $pays_status = $m->where(array("pays_orderid" => $TransID))->getField("pays_status");
        if (intval($pays_status) == 1 || intval($pays_status) == 3) {
    ...
    public function checkstatus()
    {
        ...
        $Order   = M("Order");
        $order   = $Order->where(array('pays_orderid' => $orderid))->find();
        if ($order['pays_status'] != 0) {
            echo json_encode(array('status' => 'ok', 'callback' => $this->_site . "Pay_" . $order['pays_tongdao'] . "_callbackurl.html?orderid="
                . $orderid . "&pays_memberid=" . $order['pays_memberid'] . '&bankcode=' . $order['pays_bankcode']));
            exit();
        } else {
            exit("no-$orderid");
        }
     ...
     * @param $pays_memberid
     */
    protected function domaincheck($pays_memberid)
    {
        $referer      = $_SERVER["HTTP_REFERER"]; // 获取完整的来路URL
        $domain       = $_SERVER['HTTP_HOST'];
        $pays_memberid = intval($pays_memberid) - 10000;
        $User         = M("User");
        $num          = $User->where(["id" => $pays_memberid])->count();
        if ($num <= 0) {
            $this->showmessage("商户编号不存在");
        } else {
            $websiteid     = $User->where(["id" => $pays_memberid])->getField("websiteid");
            ...
            if ($websitedomain != $domain) {
                ...
                $domains        = $Userverifyinfo->where(["userid" => $pays_memberid])->getField("domain");
                if (!$domains) {
    ...
    protected function getParameter($title, $channel, $className, $exchange = 1)
    {
        ...
        $parameter = array(
            ...
            'gateway'      => '',
            'orderid'      => '',
            'out_trade_id' => I('request.pays_orderid', ''), //外部订单号
            'channel'      => $channel,
     ...
     * @date: 2020/7/8 13:45
     */
    public function check_black($channel_id){
        $channel = M('Channel')->where(['id' => $channel_id])->find();
        //来路域名
        $url = $_SERVER["HTTP_REFERER"];   //获取完整的来路URL
        $str = str_replace(["http://","https://"],"",$url);  //去掉http://
        $strdomain = explode("/",$str);               // 以“/”分开成数组
        $domain = $strdomain[0];              //取第一个“/”以前的字符
        //来路IP
        $ip = get_client_ip();
        $whit_ips = array_filter(explode("|", $channel['white_list']));
        $whit_domains = array_filter(explode("|", $channel['white_domain']));

         //条件判断，IP或是域名正确进入下一步
        if(in_array($domain,$whit_domains) || in_array($ip,$whit_ips)){
          //
        }else{
            return false;
        }
