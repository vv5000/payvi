...
class IpsPaySubmit
{
    ...
    function __construct($ipspays_config){
        $this->ipspays_config = $ipspays_config;
    }
    function IpsPaySubmit($ipspays_config) {
        
        $this->__construct($ipspays_config);
    }
    /**
     * 建立请求，以表单HTML形式构造（默认）
    ...
    function buildRequestForm($para_temp) {
        ...
        $sHtml = "<form id='ipspaysubmit' name='ipspaysubmit' method='post' action='".$this->ipspays_config['PostUrl']."'>";
    ...
    function buildHead($para_temp){
        ...
        $sReqXmlHead .= "<Signature>".hxMd5Sign($this->buildBody($para_temp),$para_temp["MerCode"],$this->ipspays_config['MerCert'])."</Signature>";
        $sReqXmlHead .= "</head>";
        return $sReqXmlHead;
    }
    /**
