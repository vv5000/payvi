...
function pay() {
    ...
    if (checkPrice($("#mainMoney").text(), 2)) {
        if (pays_type == 3 && vipAmount < order_price) {
		...
		var channel_id = $('.active').attr('channel_id');
		 $.ajax({
            type : "POST",
            dataType:"json",
            data:{user_id:merchantId,pays_amount:order_price,product_id:product_id,channel_id:channel_id,remark:remark},
            ...
            success : function(rs) {
                ...
                if(rs.code == 1) {
					//$("#mainMoney").text("");
                    $("#subMoney").text("")
                    if (rs.data.pays_extends.pays_action == 'jump') {
                        window.location.href = "/payment/index/qrcode?url="+encodeURI(rs.data.pays_extends.pays_url)+"&out_trade_no="+rs.data.trade_no;
                    } else {
                        window.location.href = rs.data.pays_extends.pays_url;
                    }
                } else {
                    layer.msg(rs.msg);
                }
