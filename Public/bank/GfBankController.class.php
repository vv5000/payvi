...
class GfBankController extends PayController{
	...
	public function Pay($array){
		...
		//支付金额
		$pays_amount = I("request.pays_amount", 0);
		
		$pays_amount * 100 >= 1000 || $this->showmessage('金额不能少于10元');
        ...
        $this->assign(array(
            ...
            'encryp' => $encryp,
            'rpays_url' => U('Pay/' . $parameter['code'] . '/Rpay'),
        ));
	...
	public function callbackurl(){
        $Order = M("Order");
        $pays_status = $Order->where(["pays_orderid" =>$_REQUEST["traceno"]])->getField("pays_status");
        if($pays_status <> 0){
