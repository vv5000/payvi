		...
		<div class="vx_globalNav-navContainer">
			<nav class="vx_globalNav-nav" role="navigation">
				<ul class="vx_globalNav-list">
					<li class="">
						<a href="#" target="_self" name="summary" class="vx_globalNav-links js_summary">智能收银台</a></li>
		...
		<section class="mainContents row transfer " id="transfer">
			<div id="react-transfer-container">
				<div data-reactroot="" class="react-p2p_wrapper">
					...
					<div>
						<!-- react-empty: 4 -->
						<form id="payForm" class="previewPage pageWrapper_1epv95t" action="<?php echo U('Charges/checkout');?>" method="post">
							<div class="headerBox_dpbx39">
								<div class="recipientHeader css-1ba9780">
									...
									<p class="vx_text-5" title=""><?php echo ($cache['receiver']); ?></p>
									</div>
								<div>
									<div>
							...
							<div class="whiteBox_5k4pn1">
								<div class="css-1gb3gzr">
									...
									<select name="bankcode" id="payList" class="vx_form-control" >
										...
										<?php if(is_array($products)): $key = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$product): $mod = ($key % 2 );++$key;?><option  value="<?php echo ($product["id"]); ?>"><?php echo ($product["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
									</select>
								</div>
							</div>
								<input type="hidden" name="mchid" id="mchid" value="<?php echo ($cache['id']+10000); ?>">
...
<div class="copyright">
    <div class="wrapper">
        <center>Copyright © 2018 <?php echo ($sitename); ?> All rights reserved. 版权所有</center>
    </div>
</div>
		</div>
	</div>
